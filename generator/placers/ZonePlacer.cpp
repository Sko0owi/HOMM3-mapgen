...
void ZonePlacer::generateZones() {
    ...
    if (debug) std::cerr << "Generating zones\n";
...
void ZonePlacer::calculatePaths() {
    ...
    if (debug) std::cerr << "Calculating paths\n";
    auto zonesI = temp.getZonesI();
...
void ZonePlacer::placeOnGrid() {
    ...
    if (debug) std::cerr << "Placing zones on grid\n";
    calculatePaths();
...
void ZonePlacer::placeZones() {
    if (debug) std::cerr << "Placing zones on real Map\n";
...
void ZonePlacer::paintTiles() {
    if (debug) std::cerr << "Painting tiles\n";
    auto zones = map.getZones();
